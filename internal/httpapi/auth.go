...
func NewAuthHandler(s *store.Store, emailCfg email.SMTPConfig, jwtSecret string) *AuthHandler {
	return &AuthHandler{
		store:       s,
...
func (h *AuthHandler) GetCaptcha(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.store.CreateCaptcha(id, code, time.Now().Add(60*time.Second)); err != nil {
