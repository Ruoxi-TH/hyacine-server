...
func newTestRouter(cfg config.Config) http.Handler {
	db, _ := store.New("./data")
	return NewRouter(cfg, db, email.SMTPConfig{}, "test-secret")
}

func TestHealth(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/api/v1/health", nil)
	res := httptest.NewRecorder()
	newTestRouter(config.Config{Port: "3000", NeteaseAPIBase: "http://127.0.0.1:3001"}).ServeHTTP(res, req)
...
func TestHealthDirectModeCapabilities(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/api/v1/health", nil)
	res := httptest.NewRecorder()
	newTestRouter(config.Config{Port: "3000"}).ServeHTTP(res, req)
